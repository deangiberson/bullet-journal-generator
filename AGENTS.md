...
- Accessible by simplicity: minimal controls, clear focus/keyboard paths, simple add/remove (big X), and undo/redo for layout edits. Keyboard palette opens with “/”; select with “s”; codes scan left→right/top→bottom; row/col placement is collected inline in the palette (no prompts).
